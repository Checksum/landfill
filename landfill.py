...
class Generator(object):
    ...
    def get_pwiz_tables(self, engine, database):
        ...
        self.db_models = imp.load_source('db_models', out_file)

        return self.get_tables(self.db_models)

    def run(self):
        for table_name, model_table in self.model_tables.iteritems():
            if table_name in self.db_tables:
                logger.debug("%s already exists in the DB. Checking fields now" % table_name)
                model_set = set(model_table._meta.fields)
                ...
                if added:
                    ...
                    for column in added:
                        self.generate_definition('add_column', model_table, table_name, column)
                if deleted:
                    logger.info("Columns deleted: %s" % deleted)
                    for column in deleted:
                        self.generate_definition('drop_column', self.db_tables.get(table_name), table_name, column)
            else:
                logger.info("%s is a new table" % table_name)
                model_class = model_table._meta.model_class.__name__
    ...
    def get_model_source(self, _type, model_name):
        if not model_name in self.source_cache:
            klass = self.models if _type == 'add_column' else self.db_models
            model_source = inspect.getsource(getattr(klass, model_name)).strip()
